...
def extract_and_render_html(email_message):
    """Extracts raw HTML and returns (raw_html, formatted_html)."""
    # Attempt to find HTML in common places
    html = ""
...
def analyze_html_structure(html_content):
    """Analyzes HTML structure and returns element counts and responsiveness info."""
    ...
    return {
        ...
        "images": {
            "count": len(images),
            "with_alt_text": len(img_with_alt),
            "without_alt_text": len(img_without_alt),
            "with_width_height": len(img_with_dim),
        },
        "responsiveness": {
...
def check_email_compatibility(html_content):
    """Checks for common compatibility issues in email HTML."""
    soup = BeautifulSoup(html_content, "html.parser")
    problematic = {
        "background_images": bool(soup.find_all(style=re.compile(r"background(-image)?:"))),
        "forms": bool(soup.find_all("form")),
        "video": bool(soup.find_all("video")),
        "javascript": bool(soup.find_all("script")),
    }

    general = {
        "has_doctype": html_content.strip().lower().startswith("<!doctype"),
        "uses_html5_elements": bool(soup.find_all(["section", "article", "header", "footer", "nav"])),
    }

    layout = {
        "uses_tables_for_layout": bool(soup.find_all("table")),
    }
